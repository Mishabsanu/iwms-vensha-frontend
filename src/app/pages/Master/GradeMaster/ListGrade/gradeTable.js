...
export default function ListGradeTable({
  searchTerm,
  page,
  setPage,
  sort,
  ...
  const { gradeMaster, TotalPage, loading } = useSelector(
  ...
  const handleItemAction = (menuItem) => {
    switch (menuItem.action) {
      case "edit":
        navigate("/master/grade/edit", { state: menuItem?.data });
        break;
      case "editStatus":
        Swal.fire({
    ...
    <>
      ...
      <TableContainer component={Paper} sx={{ width: "100%" }}>
        <Table size="small">
          ...
          <TableBody>
            {gradeMaster?.map((row, i) => (
              <TableRow key={i}>
                <TableCell sx={{ textAlign: "left" }}>
                  {row.grade_name}
                </TableCell>
                <TableCell sx={{ textAlign: "left", px: 1 }}>
                  {row?.grade_remarks ? row?.grade_remarks : "-"}
                </TableCell>
                <TableCell
