...
import { addUnLoading } from "app/services/apis/addUnLoading";
import { updateUnLoading } from "app/services/apis/updateUnLoading";
...
export default function AddUnLoading() {
  ...
  const validationSchema = yup.object({
    ...
    po: yup.string("Enter po").required("po is required"),
    truck_number: yup
  ...
  const onVehicleSave = async (values) => {
    const body = { ...values };
    setSubmitting(true);
    if (pathname === "/master/unloading/edit") {
      const data = await updateUnLoading(body, state._id);
      if (data?.data?.status === true) {
        Swal.fire({
          icon: "success",
          title: "UnLoading Edited Successfully",
          text: "",
          timer: 1000,
          showConfirmButton: false,
        });
        navigate("/dashboard/master/unloading");
      } else {
        Swal.fire({
          icon: "error",
          title: data?.message,
      ...
      const data = await addUnLoading(body);
      if (data?.data?.status === true) {
        Swal.fire({
          icon: "success",
          title: "UnLoading Added Successfully",
          text: "",
          timer: 1000,
          showConfirmButton: false,
        });
              ...
              <Div sx={{ mt: 4 }}>
                ...
                >
                  <Grid container rowSpacing={3} columnSpacing={3}>
                    <Grid item xs={4}>
                      <Typography variant="h5">UnLoading Date</Typography>
                    ...
                    <Grid item xs={4}>
                      <FormTextField1 name="po" label="PO*" />
                    </Grid>
                    <Grid item xs={4}>
                      <FormTextField1
                        name="truck_number"
