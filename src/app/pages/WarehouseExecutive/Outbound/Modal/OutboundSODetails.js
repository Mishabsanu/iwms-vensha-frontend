...
export const OutboundSODetails = ({ open, setOpen, rowData, onSelect }) => {
  ...
  const modifyList = outbound.filter(
    (item) => item.sales_order_no === rowData.sales_order_no
  );
              ...
              {modifyList?.map((item) =>
                item?.skus.map((row) => (
                  <TableRow key={item?._id} sx={{ bgcolor: "#EDEBEB" }}>
                    ...
                    <TableCell sx={{ px: 1 }}>{item?.customer_name}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
