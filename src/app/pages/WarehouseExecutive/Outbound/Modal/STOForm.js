import { Grid, FormControl, MenuItem, TextField, Typography, IconButton } from "@mui/material";
import { FieldArray } from "formik";
import AddIcon from "@mui/icons-material/Add";
import RemoveIcon from "@mui/icons-material/Remove";

const STOForm = ({ values, setFieldValue, errors, touched }) => (
  <>
      ...
      <Grid item xs={12} sm={6} md={4} lg={4} xl={4}>
        <FormControl fullWidth>
          ...
          >
            ...
            {/* Replace with actual plant data */}
          </TextField>
        </FormControl>
      </Grid>
    </Grid>
          ...
          {values.skus.map((sku, index) => (
            <Grid container spacing={2} key={index} alignItems="center">
              ...
              <Grid item xs={12} sm={2}>
                <IconButton onClick={() => remove(index)} disabled={values.skus.length === 1}>
                  <RemoveIcon />
                </IconButton>
                {index === values.skus.length - 1 && (
                  <IconButton onClick={() => push({ sku_code: "", sku_description: "", sut: "", stock_qty: "" })}>
                    <AddIcon />
                  </IconButton>
                )}
              </Grid>
